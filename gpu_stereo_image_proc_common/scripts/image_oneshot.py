...
class CameraInfoPub:
    def __init__(self,arg):
        ...
        self.msg.D = yaml_to_np(camera_params["distortion_coefficients"]).flatten().tolist()
        self.msg.R = yaml_to_np(camera_params["rectification_matrix"]).flatten().tolist()
...
class OutputImageSub:
    ...
    def __init__(self, arg):
        ...
        rospy.loginfo("Waiting for image on topic %s, save to \"%s\"" % (self.topic, self.filename))
        rospy.Subscriber(self.topic, Image, self.callback)

    def callback(self, msg):
        rospy.loginfo("Received image on topic %s, encoding as %s, saving to \"%s\"" % (self.topic,self.encoding,self.filename))
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(
                    prog = 'image_oneshot.py',
                    description = 'What the program does')

    parser.add_argument('--input-image', 
                    action='append',
                    help='Input image topics')
    ...
    rospy.loginfo("Sending %d image and %d camera info topics" % (len(input_images), len(camera_info)))
